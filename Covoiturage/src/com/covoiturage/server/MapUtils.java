...
public class MapUtils {
	...
	public static List<Journey> journeyNearBuffer(Geometry buffer, float distanceJourney, Date departureStart, Date departureEnd, Date arrival){
		...
		try
		{
			...
			for (Journey journey : results) {
				...
				if(geom.buffer(distanceJourney/111).contains(buffer)){
					...
					float duration=0;
					try{
						String urlString = "http://routes.cloudmade.com/8ee2a50541944fb9bcedded5165f09d9/api/0.3/"+array[0].x+","+array[0].y+",%5B";
						for(int l=0; l<arrayWaypoints.length;l++){
							urlString+=arrayWaypoints[l].x+","+arrayWaypoints[l].y+",";
						}
						urlString=urlString.substring(0,urlString.length()-2);
						urlString+="%5D,"+array[array.length-1].x+","+array[array.length-1].y+"/car/fastest.gpx";
					...
					if(journey.getArrival().getTime()<=arrival.getTime()&& journey.getDepartureEnd().getTime()>=departureStart.getTime()&&distance<=distanceJourney &&(new Date(journey.getDepartureStart().getTime()+(long)(duration*1000))).getTime()<=journey.getArrival().getTime()){
						journey.getSteps();
	...
	public static List<SimpleTravel> bufferRoute(List<String> coordinates, Date departureStart, Date departureEnd, Date arrival, float distance){
		...
		Geometry polyline= new GeometryFactory().createLineString(coordArray);
		return simpleTravelsInBuffer(coordArray,polyline.buffer(distance/111),departureStart,  departureEnd,  arrival, distance);
	}

	public static List<SimpleTravel> simpleTravelsInBuffer(Coordinate[] array,Geometry buffer, Date departureStart, Date departureEnd, Date arrival, float distanceJourney){
		...
		try
		{
			...
			for (SimpleTravel travel : results) {
				...
				if(buffer.contains(new GeometryFactory().createMultiPoint(coordArray))){
					...
					float duration=0;
					try{
						URL url = new URL("http://routes.cloudmade.com/8ee2a50541944fb9bcedded5165f09d9/api/0.3/"+array[0].x+","+array[0].y+",%5B"+coordArray[0].x+","+coordArray[0].y+","+coordArray[coordArray.length-1].x+","+coordArray[coordArray.length-1].y+"%5D,"+array[array.length-1].x+","+array[array.length-1].y+"/car/fastest.gpx");
						BufferedReader reader = new BufferedReader(new InputStreamReader(url.openStream()));
						String line;
					...
					if(travel.getArrival().getTime()<=arrival.getTime()&& travel.getDepartureStart().getTime()<=departureEnd.getTime() && distance<=distanceJourney &&(new Date(departureStart.getTime()+(long)(duration*1000))).getTime()<=arrival.getTime())
